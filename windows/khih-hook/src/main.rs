//! khih-hook: the minimal client Claude Code's hooks call.
...
/// Pulls "port": N out of %APPDATA%\khih\config.json (hand-rolled scan, no dependency)
fn read_port() -> u16 {
    let path = match std::env::var("APPDATA") {
        Ok(a) => format!("{a}\\khih\\config.json"),
