//! `khih.exe doctor` — self-diagnosis: look instead of guessing.
//! Checks the config, port occupancy, watch roots, the newest session file and how its tail parses,
//! and writes to stdout plus %APPDATA%\khih\doctor.log.
...
pub fn run() -> String {
    ...
    match std::net::TcpListener::bind(("127.0.0.1", cfg.port)) {
        Ok(_) => o += "port: free — no Khih instance is running\n",
    ...
    if let Some(dir) = dirs::config_dir() {
        let p = dir.join("khih").join("watch.log");
