//! Merges khih-hook.exe into ~/.claude/settings.json without overwriting the user's own hooks.
//! Identification: the command contains "khih-hook". A backup is written first.

use serde_json::{json, Value};
use std::path::PathBuf;

/// (Claude Code event name, whether it needs a matcher, the internal event reported to Khih)
    ...
    entry["hooks"]
        ...
        .map(|hs| {
            hs.iter().any(|h| {
                h["command"]
                    .as_str()
                    .map(|c| c.contains("khih-hook") || c.contains("eatbean-hook") || c.contains("pacman-hook"))
                    .unwrap_or(false)
            })
        })
        .unwrap_or(false)
...
fn backup_and_write(path: &PathBuf, root: &Value) -> Result<(), String> {
    ...
    if path.exists() {
        let ts = std::time::SystemTime::now()
            ...
            .map(|d| d.as_secs())
            .unwrap_or(0);
        let _ = std::fs::copy(path, path.with_extension(format!("json.khih-bak-{ts}")));
    }
...
pub fn uninstall() -> Result<String, String> {
    ...
    backup_and_write(&path, &root)?;
    Ok(format!("removed {removed} Khih hook(s)"))
}
