...
fn ack_scan(app: &AppHandle) -> bool {
    ...
    let fg_is_claude_desktop = fg_name.contains("claude") && !fg_name.contains("khih");
    let st = app.state::<AppState>();
    let mut store = st.store.lock().unwrap();
    store.ack_done(|s| {
        if s.ppid == 0 {
...
fn main() {
        ...
        match cmd.as_str() {
            ...
            "autostart" => {
                let r = match args.get(2).map(|s| s.as_str()) {
                    ...
                    _ => Err("usage: khih.exe autostart on|off".into()),
                };
                report(r);
                return;
            }
    ...
    tauri::Builder::default()
        .plugin(tauri_plugin_single_instance::init(|app, _args, _cwd| {
            ...
            let _ = app.emit("notice", format!("Khih is already running ({BUILD}) — quit it from the tray before starting a new build"));
        }))
        .manage(AppState {
        ...
        .setup(move |app| {
            ...
            // Persist the config (khih-hook reads the port from it)
            {
