...
- 左鍵由 `NotchPanel.sendEvent` 統一路由；SwiftUI／輔助使用共用動作，避免一次按下執行兩次。
  保留設定按鈕、釘住與 Option 拖曳。`detailsOnClick` 預設 true，懸停不開／切換明細；
  外觀關閉該選項才恢復 hover。`detailsShowRemaining` 預設 false（已用），切換詳細頁 bar＋
  百分比，不改 ring、警告色依據或引擎證據。兩選項保存，現有／新螢幕皆同步套用。
...
- 新增 Codex 用設定視窗的原生 sheet：命名→瀏覽器登入→完成，代碼可選取／複製。
  失敗保留名稱供重試；重複登入明說未新增，不觸發新增 provider／monitor。
  取消／Escape／關閉／啟動失敗／逾時，都須等程序與暫存帳號清理後才結束或接受下一次登入。
- 點擊手感與 backend 等待分開：縮入 0.93 倍、380ms 後釋放，保留原本有限旋轉／spring；
  等待短弧用獨立 `TimelineView` 持續轉，完成移除即停止，不能一圈後停住或永遠縮小。
  額度變化、旋轉、縮放分別限定動畫範圍；Reduce Motion 靜態提示，不旋轉／縮放。
  共享進行中回饋沿用 `UsageStore.refreshing`；frame 未變不要 `setFrame(display: true)` 重繪整窗。
...
- `build/` 被 gitignore；`make clean` 刪整個 build，`make build-ci` 也會先刪 `build/ci`。
  `Codenotch.app` 在這裡只是可重建產物，不可宣稱已安裝或永久存在；要長期保留須明確複製到
  `/Applications`，覆寫前先取得同意。找 App 先查實際路徑、DerivedData／Applications，不信舊進度文字。
...
- 渲染環境用 `TEST_RUNNER_GROUPED_RENDER_DIR`／`TEST_RUNNER_RING_RENDER_DIR` 傳入 test host；
  單設外層同名變數不保證傳入。原生文字欄位用 NSHostingView＋AppKit bitmap 與明確背景，
  ImageRenderer 的禁止符號／透明黑底圖不能當通過。唯讀 accessibility environment 不硬寫入。
...
- AX 失效／screenshot unavailable 不能當成介面驗收成功；程式測試、渲染、實機驗收分開回報。
  ...
  notch 收合且無法 hover 時交使用者看，不要拿空截圖當通過。
