...
///
/// Worth showing plainly, because Khih never signs in — it borrows a
...
///
/// Khih cannot sign anyone in — it reads a credential the owning tool
...
enum SignInRoute: Equatable {
    ...
    /// The only case where Khih genuinely signs anyone in or out.
    ...
    var signOutCaveat: String {
        switch self {
        case .modal(let name):
            return L10n.t("Signs out of \(name) — the session belongs to Khih.")
...
struct ProviderSummary: Identifiable, Equatable {
    ...
    /// Whether this provider's saved login has aged out and Khih could not
